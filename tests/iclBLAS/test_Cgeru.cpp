...
TEST(Cgeru, naive_3x5_incx2) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            ACCESS(expected, j, i, lda) += alpha * x[j*incx] * y[i*incy];
        }
    }
    ...
    status = iclblasCgeru(handle, m, n, reinterpret_cast<oclComplex_t*>(&alpha), reinterpret_cast<oclComplex_t*>(x), incx,
