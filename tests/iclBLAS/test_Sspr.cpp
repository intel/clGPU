...
TEST(Sspr, naive_test_upper) 
{
    ...
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < n; ++j)
        {
            float res_1 = alpha * x[i] * x[j];

            if(j >= i)
                US_ACCESS(ex_result, i, j, n) = res_1 + US_ACCESS(AP, i, j, n);
        }
    }
    ...
    status = iclblasSspr(handle, uplo, n, &alpha, x.data(), incx, AP.data());
    ASSERT_EQ(status, ICLBLAS_STATUS_SUCCESS);
...
TEST(Sspr, naive_test_lower) 
{
    ...
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < n; ++j)
        {
            ...
            if (i >= j)
                LS_ACCESS(ex_result, i, j, n) = res_1 + LS_ACCESS(AP, i, j, n);
        }
    }
