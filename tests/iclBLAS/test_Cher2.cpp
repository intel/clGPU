...
TEST(Cher2, naive_up_5x5_1incs) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i; j < n; j++) {
            ACCESS(expected, i, j, lda) += alpha * (x[i*incx] * std::conj(y[j*incy]));
            ACCESS(expected, i, j, lda) += std::conj(alpha) * (y[i*incy] * std::conj(x[j*incx]));
        }
    ...
    status = iclblasCher2(handle, uplo, n, reinterpret_cast<oclComplex_t*>(&alpha), reinterpret_cast<oclComplex_t*>(x), incx,
...
TEST(Cher2, naive_up_3x3_2incs) {
    ...
    std::complex<float> a[n*lda] = { { 1.f, 2.f }, { -1.f, -2.f }, { -3.f, -4.f },
...
TEST(Cher2, naive_low_5x5_1incs) {
...
TEST(Cher2, naive_low_4x3_2incx) {
    const auto uplo = ICLBLAS_FILL_MODE_LOWER;
    const int n = 3;
    const int incx = 2;
    const int incy = 1;
