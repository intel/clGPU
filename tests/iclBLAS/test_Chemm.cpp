...
TEST(Chemm, naive_left_up_3x3) {
        ...
        for (int j = 0; j < m; j++) {
            ...
            for (int k = 0; k < m; k++) {
                if (k > j) {
                    ...
                    std::complex<float> diag_a = ACCESS(a, j, k, lda);
                    diag_a.imag(0.f);
                    axb += diag_a * ACCESS(b, k, i, ldb);
                }
                else {
                    axb += std::conj(ACCESS(a, k, j, lda)) * ACCESS(b, k, i, ldb);
                }
            }
    ...
    status = iclblasChemm(handle, side, uplo, m, n, reinterpret_cast<oclComplex_t*>(&alpha), reinterpret_cast<oclComplex_t*>(a), lda,
        ...
        for (int j = 0; j < m; j++) {
            ...
            for (int k = 0; k < n; k++) {
                if (k < i) {
                    ...
                    std::complex<float> diag_a = ACCESS(a, k, i, lda);
                    diag_a.imag(0.f);
                    bxa += diag_a * ACCESS(b, j, k, ldb);
                }
                else {
                    bxa += std::conj(ACCESS(a, i, k, lda)) * ACCESS(b, j, k, ldb);
                }
            }
...
TEST(Chemm, naive_right_low_3x3_lds4) {
    const auto side = ICLBLAS_SIDE_RIGHT;
    const auto uplo = ICLBLAS_FILL_MODE_LOWER;
    const int m = 3;
    const int n = 3;
