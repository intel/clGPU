...
/* Copyright (c) 2016-2018 Intel Corporation
 ...
 limitations under the License.
   ...
   This kernel uses a tile-size = 16x2. This is the size of output-tile computed by each work-item.
   Kernel Configuration given below 
   
   local_workgroup_size = (8,1,1)
   global_grid_size    = (N/2,M/16,1)
   ...
   Kernel Arguments: 
          ...
          Other arguments - refer to kernel declaration ( __kernel void gemm_buf_16x2 )
...
void gemm_16x16(const __global float *A,
              ...
              uint local_x)
{
    ...
    */
    ...
    intel_sub_group_block_write2(&C_write[14*N_], as_uint2(C14));
    intel_sub_group_block_write2(&C_write[15*N_], as_uint2(C15));
}

__attribute__((reqd_work_group_size(8, TY, 1)))
__attribute__((intel_reqd_sub_group_size(8)))
__kernel void gemm_buf_16x2 (const __global float *A,
                             __read_only image2d_t B,
                             __global float *C,
                             const float alpha,
                             const float beta,
...
#define local_x get_local_id(0)
    ...
    gemm_16x16(A, B, C, alpha, beta, K, N, M, group_x, group_y, local_x);
