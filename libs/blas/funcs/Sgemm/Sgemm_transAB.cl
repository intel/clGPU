...
__kernel void Sgemm_transAB(uint k, float alpha, __global float* A, uint lda, __global float* B, uint ldb, float beta, __global float* C, uint ldc)
{
    ...
    for(uint i = 0; i < k; i++)
    {
        uint indexA = row * lda + i;
        float valueA = A[indexA];

        uint indexB = i * ldb + column;
