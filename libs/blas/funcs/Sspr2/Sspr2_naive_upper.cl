...
__kernel void Sspr2_naive_upper(int n, float alpha, __global float* x, int incx, __global float* y, int incy, __global float* AP)
{
    ...
    if(col_id >= row_id)
    {
        ...
        US_ACCESS(AP, row_id, col_id, n) = res_1 + res_2 + US_ACCESS(AP, row_id, col_id, n);
    }
}
