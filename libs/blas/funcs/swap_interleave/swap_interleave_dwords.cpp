...
static const char* module_name = "swap_interleave_dwords";
static const char* kernel_name = "swap_interleave_dwords";

namespace iclgpu { namespace functions { namespace implementations {

bool swap_interleave_dwords::accept(const swap_interleave::params& params, swap_interleave::score& score)
{
    if (params.elem_size != 4) return false;
    score.elem_size = 1.1f;
    return true;
}

event swap_interleave_dwords::execute(const swap_interleave::params& params, const std::vector<event>& dep_events)
{
