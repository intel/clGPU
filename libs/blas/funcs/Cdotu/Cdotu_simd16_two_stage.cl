...
__kernel void Cdotu_simd16_first_stage(int n, __global complex_t* x, int incx, __global complex_t* y, int incy, __global complex_t* result)
{
...
__attribute__((intel_reqd_sub_group_size(SIMD)))
__attribute__((reqd_work_group_size(SIMD, SIMD_PER_GROUP, 1)))
__kernel void Cdotu_simd16_second_stage(__global complex_t* x, __global complex_t* result) {
    const uint lid = get_sub_group_local_id();
    const uint sid = get_sub_group_id();
