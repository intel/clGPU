...
__kernel void Ssyr2_naive_lower(uint n, float alpha, const __constant float* x, int incx, const __constant float* y, int incy, __global float* A, uint lda)
{   
    for (uint row_id = 0; row_id < n; ++row_id)
    {
        for (uint col_id = 0; col_id <= row_id; ++col_id)
        {
