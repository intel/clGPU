...
__kernel void Ssyr2_naive_lower(int uplo, int n, float alpha, __global float* x, int incx, __global float* y, int incy, __global float* A, int lda,
 __global float* tempMatrix1, __global float* tempMatrix2, __global float* tempMatrix, __global float* tempX, __global float* tempY)
{
    ...
    for(i = 0; i<n; ++i)
    {
        for(j = 0; j<n; ++j)
        {
            if(i>= j) A[i*lda + j] = tempMatrix[i*lda + j] + A[i*lda + j];
        }
    }
