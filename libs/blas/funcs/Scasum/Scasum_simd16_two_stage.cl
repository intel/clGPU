...
__kernel void Scasum_simd16_two_stage_1(int n, __global complex_t* x, int incx, __global float* result)
{
...
__attribute__((intel_reqd_sub_group_size(SIMD)))
__attribute__((reqd_work_group_size(SIMD, SIMD_PER_GROUP, 1)))
__kernel void Scasum_simd16_two_stage_2(__global float* x, __global float* result)
{
    const uint lid = get_sub_group_local_id();
    const uint sid = get_sub_group_id();
