...
__kernel void Sgemv_opt_simd16_TC(uint trans, uint m, uint n, float alpha, __global float* A, uint lda, __global float* x, int incx, float beta, __global float* y, int incy)
{
    ...
    for (uint col_loop_id = col_id; col_loop_id < m; col_loop_id += SIMD_WIDTH)
        thread_res = fma(MAT_ACCESS(A, row_id, col_loop_id, lda), x[col_loop_id * incx], thread_res);
