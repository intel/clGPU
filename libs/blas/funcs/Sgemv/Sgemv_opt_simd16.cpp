...
static const char* module_name = "Sgemv_opt_simd16";
static const char* kernel_name = "Sgemv_opt_simd16";
...
bool Sgemv_opt_simd16::accept(const Sgemv::params& params, Sgemv::score& score)
{
    if (params.n > SIMD_WIDTH && params.trans == ICLBLAS_OP_N && (params.incx > 0 && params.incy > 0))
    {
        score.n = 3.0f;
        score.trans = 1.1f;
        return true;
...
event Sgemv_opt_simd16::execute(const Sgemv::params& params, const std::vector<event>& dep_events)
{
    ...
    nd_range gws(params.m, SIMD_WIDTH);
