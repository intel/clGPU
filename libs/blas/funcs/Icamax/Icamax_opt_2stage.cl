...
__kernel void Icamax_opt_2stage(int lengthh, __global complex_t* buffer, __global int* result)
{
 ...
 accumulator.value = scabs1(buffer[global_index]);

 __attribute__((opencl_unroll_hint))
  while (global_index < lengthh) 
  {
      iav_type element;
      element.value = scabs1(buffer[global_index]);
