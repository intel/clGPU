...
__kernel void Isamax_opt_simd16_2stage(int n, __global float* x, int incx, __global float2* tempResult)
{
...
__attribute__((intel_reqd_sub_group_size(SIMD_WIDTH)))
__attribute__((reqd_work_group_size(LOCAL_GROUP_SIZE, 1, 1)))
__kernel void Isamax_opt_simd16_2stage_2(uint n, __global float2* tempRes, __global int* result)
{
    uint subgr_id = get_sub_group_id();
    uint subgr_local_id = get_sub_group_local_id();
    uint global_id = get_global_id(0);
