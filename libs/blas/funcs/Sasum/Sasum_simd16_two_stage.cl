...
__kernel void Sasum_simd16_first_stage(uint n, __global float* x, uint incx, __global float* result)
{
    ...
    const uint id = simd_lid + simd_id*SIMD_PER_GROUP + group_id*GROUP_TILE;
    uint ind = id*incx;
...
__attribute__((reqd_sub_group_size(SIMD)))
__attribute__((reqd_work_group_size(SIMD, SIMD_PER_GROUP, 1)))
__kernel void Sasum_simd16_second_stage(__global float* x, __global float* result)
{
    __local float group_sum[SIMD_PER_GROUP];
    const uint simd_id = get_sub_group_id();
    const uint simd_lid = get_sub_group_local_id();
