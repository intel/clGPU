...
__kernel void Ssyr2k_naive(int uplo, int trans, int n, int k, float alpha, __global float* a, int lda, __global float* b, int ldb, float beta, __global float* c, int ldc)
{
    ...
    if (ntrans) {
        for (int i=0; i<k; i++) {
            value += ACCESS(a, ind_m, i, lda)*ACCESS(b, ind_n, i, ldb);
            value += ACCESS(b, ind_m, i, ldb)*ACCESS(a, ind_n, i, lda);
        }
    } else {
         for (int i=0; i<k; i++) {
            value += ACCESS(a, i, ind_m, lda)*ACCESS(b, i, ind_n, ldb);
            value += ACCESS(b, i, ind_m, ldb)*ACCESS(a, i, ind_n, lda);
        }
    }
