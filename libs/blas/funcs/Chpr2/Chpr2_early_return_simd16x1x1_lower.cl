...
__kernel void Chpr2_early_return_simd16x1x1_lower(uint n, complex_t alpha, __global complex_t* x, uint incx, __global complex_t* y, uint incy, __global complex_t* a)
{
    ...
    uint index_a = IDX_PACKED_LOWER(ind_m, group_start_n, n);
    ...
    #define NEXT_COL(iteration) (n - group_start_n - (iteration) - 1)
    ...
    if (last_row && diagonal) {
        ...
        for (uint tile_col = 0; tile_col < SIMD; tile_col++) {
            // Separated shuffling and updating, due to the fact that work items in exceeding rows
            // hold values for columns inside matrix, so they need to encounter this shuffle
            complex_t prod1 = cmul(left_x, intel_sub_group_shuffle(right_y, tile_col));
            complex_t prod2 = cmul(left_y, intel_sub_group_shuffle(right_x, tile_col));
