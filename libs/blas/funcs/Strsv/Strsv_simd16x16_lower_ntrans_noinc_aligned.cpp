...
static const char* module_name = "Strsv_simd16x16_lower_ntrans_noinc_aligned";
static const char* kernel_name = "Strsv_simd16x16_lower_ntrans";
...
bool Strsv_simd16x16_lower_ntrans_noinc_aligned::accept(const Strsv::params& params, Strsv::score& score)
{
    if (params.uplo != ICLBLAS_FILL_MODE_LOWER || params.trans != ICLBLAS_OP_N || params.incx != 1 || !is_aligned<16, float>(params.x))
        return false;
    score.uplo = 1.1f;
    score.trans = 1.1f;
    score.incx = 1.2f;
...
event Strsv_simd16x16_lower_ntrans_noinc_aligned::execute(const Strsv::params& params, const std::vector<event>& dep_events)
{
