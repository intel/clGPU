...
static const char* module_name = "Sgbmv_simd_one_row_trans_noincx";
static const char* kernel_name = "Sgbmv_simd_one_row_trans";
...
bool Sgbmv_simd_one_row_trans_noincx::accept(const Sgbmv::params& params, Sgbmv::score& score)
{
    if (params.trans == ICLBLAS_OP_N || params.incx != 1) return false;
    score.trans = 1.2f;
    score.incx = 1.1f;
    return true;
}

event Sgbmv_simd_one_row_trans_noincx::execute(const Sgbmv::params& params, const std::vector<event>& dep_events)
{
    ...
    auto gws = nd_range(params.n * simd);
