...
static const char* module_name = "Srotm_packed_diagonal_ones";
...
bool Srotm_packed_diagonal_ones::accept(const Srotm::params& params, Srotm::score& score)
{
    if (params.param[0] != 0.f || params.n < min_size) return false;
    score.param = 1.1f;
    score.n = 1.1f;
    return true;
}

event Srotm_packed_diagonal_ones::execute(const Srotm::params& params, const std::vector<event>& dep_events)
{
