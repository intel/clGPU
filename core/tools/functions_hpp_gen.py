    ...
    for impl_name in func['implementations']:
        ...
        if not os.path.exists(cl_file_path):
            with open(cl_file_path, 'w') as f:
                f.write(cl_impl_tpl.substitute(func_name=func_name, impl_name=impl_name, params=', '.join(params)))
...
functions_def_file = sys.argv[1] if len(sys.argv) > 1 else os.path.join(os.getcwd(), 'functions.def')
